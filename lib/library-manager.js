...
class LibraryManager {
  async getLibraries(userId) {
    ...
    const { rows } = await client.query(`SELECT L."id", L."name", L."description", COUNT(B."library_id") as "booksCount"
  ...
  async createLibrary(userId, library) {
    ...
    const id = uuidv4();
    await client.query(`INSERT INTO "${schemaName}"."library" ("id", "user_id", "name", "description") VALUES ($1, $2, $3, $4);`, [id, userId, name.trim(), description.trim()]);
    client.release();
    await pool.end();
    return {
      id,
  ...
  async updateLibrary(userId, library) {
    ...
    const client = await pool.connect();
    const { name, description } = library;
    const { rowCount } = await client.query(`UPDATE "${schemaName}"."library" SET "name"=$1, "description"=$2 WHERE "id"=$3 AND "user_id"=$4;`, [name.trim(), description.trim(), library.id, userId]);
    client.release();
    await pool.end();
    return (rowCount === 1);
  }
  ...
  async deleteLibrary(userId, libraryId) {
    ...
    try {
      ...
      const { rowCount } = await client.query(`DELETE FROM "${schemaName}"."library" WHERE "id"=$1 AND "user_id"=$2;`, [libraryId, userId]);
      if (rowCount === 1) {
