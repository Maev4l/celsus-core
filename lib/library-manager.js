...
class LibraryManager {
  ...
  async createLibrary(userId, library) {
    ...
    const id = uuidv4();
    await client.query('INSERT INTO "celsus"."library" ("id", "user_id", "name", "description") VALUES ($1, $2, $3, $4);', [id, userId, name.trim(), description.trim()]);
    client.release();
    await pool.end();
    return {
      id,
  ...
  async updateLibrary(userId, library) {
    ...
    const client = await pool.connect();
    const { name, description } = library;
    const { rowCount } = await client.query('UPDATE "celsus"."library" SET "name"=$1, "description"=$2 WHERE "id"=$3 AND "user_id"=$4;', [name.trim(), description.trim(), library.id, userId]);
    client.release();
    await pool.end();
    return (rowCount === 1);
  }
