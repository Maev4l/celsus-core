  ...
  async updateBook(userId, book) {
    ...
    try {
      ...
      const { rowCount } = await client.query(
        ...
        [book.id, userId, libraryId, title.trim(), description.trim(), isbn10.trim(), isbn13.trim(), thumbnail, authors.map(author => author.trim()).join(';'), tags.map(tag => tag.trim()).join(';'), hash],
      );
      await client.query('COMMIT');
      return (rowCount === 1);
    } catch (e) {
